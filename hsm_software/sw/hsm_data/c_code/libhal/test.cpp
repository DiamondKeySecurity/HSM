...
int main()
{
    ...
    rpc_serial_stream serial_stream("/dev/ttyUSB0", 921600);
    serial_stream.start_read_thread();

    std::shared_ptr<SafeQueue<rpc_packet>> myqueue = std::shared_ptr<SafeQueue<rpc_packet>>(new SafeQueue<rpc_packet>());

    rpc_packet outpacket(2 * sizeof(uint32_t));
    outpacket.encode_int(0);
    outpacket.encode_int(0);

    serial_stream.write_packet(outpacket, 0, myqueue);

    rpc_packet inpacket = std::move(myqueue->dequeue());
    uint32_t code;
    uint32_t client;
    uint32_t result;
    uint32_t version;
